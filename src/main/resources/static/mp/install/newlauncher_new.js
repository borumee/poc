  ...
  if(bWebsocketUse)
  {
	...
	var openurl = szprotocol + "//127.0.0.1";  				//고정(변경불가) https 를 사용 할 경우 https://127.0.0.1 로 변경 
  ...
  ****************************************************/  
  ...
  // var strUpdaterExePath = "MU"; //1,1,0,34 패치로 MU,MI만 설정가능
	// var strShortcutName = "MPtest320U";                                    //바로가기 이름      
	// var strKey = "test_320U";                                         //key값(필수)
	...
	// [ 3.3 버전 start ~! ]
	var strBaseURL = window.location.protocol + "//" + window.location.host;
	var strExePath = "%TOBE%\\MiPlatform330U\\MiPlatform330U.exe";
	
	// var strUpdaterExePath = "%TOBE%//MiPlatform330U//MiPlatform330U.exe"; // 1,1,0,34 버전 패치로 사용불가 
  // var strUpdaterExePath = "http://naver.com"; // 1,1,0,34 버전 패치로 URL 사용불가 	 
	var strUpdaterExePath = "MU"; //1,1,0,34 패치로 MU,MI만 설정가능
	 
	var strShortcutName = "MPtest330U";
	var strKey = "test_330U";
	var strStartXML = strBaseURL + "/mp/Demo/demo_ci_main_Win32.xml"; //Miplatform에서 사용할 StartXML의 경로(필수)                                 
	var strCommand = "-L TRUE -R FALSE -Wd 1024 -Ht 768 -D WIN32U -V 3.3 -K " + strKey + " -X " + strStartXML;
	var strVersion = "3.3";
	var bOnlyOne = false; //새창으로 시스템 열기
	//var bOnlyOne = true; //열려있으면 기존 시스템 유지
	var intTimeout = 60;
	var bPreVersionCompare = false;
	// [ 3.3 버전 end ~! ]   


 	var strUpdateURL = strBaseURL+"/mp/install/update_vista_config.xml"; //update info file의 URL(필수)
	...
	var strStartImage = strBaseURL + "/mp/install/image/next_start.gif"; //Miplatform 용 splash화면에 표시될 이미지
	...
	//var objParam = getParameter();
	
	function getParameter(name) {
	    var rtnval = '';
	    var nowAddress = window.location.href;
	...
	//console.log(" >>>>>>>> " + decodeURI(getParameter('param')))
		
	//var strGlobalVal = "a=TEST1234 ";
	var strGlobalVal;
	strGlobalVal = getParameter('id') + "," + decodeURI(getParameter('param'));
//	var strIConPath = strBaseURL +"/MP330U/install/etc_files/icon_next.ico"; //바로가기의 아이콘 파일(url 설정시 win11 os 보안강화로 처리 되지 않음)
	var strIConPath = "%APPDATA%icon_next_M_1.ico"; //바로가기의 아이콘 파일      

  var strLocLuncherVer; //런처 local 버전
  var strLauncherVersion = "1,1,0,42"; //런처 버전  
  var strLauncherDownloadPath = "/mp/install/newlauncher"; // 런처 서버 다운로드 경로
  ...
  var nRetry = 0;
    ...
    */
    ...
    function download_TPLSvc(type) {
      ...
      if (type == "newlanucher") {
        var downURL = strBaseURL + strLauncherDownloadPath + "/" + strLauncherFileName;
        return downURL;
      } else {
        // 추가 다운로드가 필요 사용 위해
      }
  ...
  var debugmode = {debug:true,loglevel:"debug"}; // loglevel debug, info, error
  ...
  {
    if(debugmode.loglevel=="debug"||debugmode.loglevel=="error") {
      trace("tpl Websocket onerror ", "first line", "action : " + objMP.action, "oepnport : "+openport);
    }
        ...
        if(isAttempt==null || isAttempt=="error") {
          ...
          */
          ...
          if (openport <= maxport) {
              //sendData(true);
              if(debugmode.loglevel=="debug"||debugmode.loglevel=="error") {
                trace("tpl Websocket onerror ", ">>>>>>>>>>>>>>>>>> Next Port Connect", "action : " + objMP.action, "oepnport : "+openport);
              }
              objWebSocket = null;
              tpl_WebSocketConnect();
          } else {
  ...
  function httpCallBackReturn(result, gubun) {
    if(result.action) {
      var action = result.action;
        ...
        if(debugmode.loglevel=="debug") {
          trace("httpCallBackReturn", "after setproperty", "action : " + objMP.action);
        }
      }
      else if(action == "method") {
        if(debugmode.loglevel=="debug") {
          trace("httpCallBackReturn", "after method", "action : " + objMP.action);
        }
      }
